...
if ($Json) {
    ...
    $updaterPath = Join-Path $env:USERPROFILE '.codex\bin\update-codex-focusfixed.ps1'
    Write-Host ("[Codex update] {0} available; current {1}." -f `
        $checkOutcome.latestVersion, $checkOutcome.currentVersion) `
        -ForegroundColor Yellow
    Write-Host ("Run: pwsh -NoProfile -ExecutionPolicy Bypass -File '{0}' -CodexVersion {1}" -f `
        $updaterPath, $checkOutcome.latestVersion) -ForegroundColor Yellow
}
