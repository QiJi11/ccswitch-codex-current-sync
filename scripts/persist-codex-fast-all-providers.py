...
def inferred_tier(config: dict) -> str:
    ...
    if service_tier in VALID_TIERS:
        candidates.append(str(service_tier))
    fast_mode = (config.get("features") or {}).get("fast_mode")
    if isinstance(fast_mode, bool):
        candidates.append("fast" if fast_mode else "standard")
    desktop_tier = (config.get("desktop") or {}).get("default-service-tier")
    if desktop_tier in VALID_TIERS:
        candidates.append(str(desktop_tier))
    distinct = set(candidates)
    if len(distinct) > 1:
        raise FastMigrationError("Global runtime tier fields disagree")
    return next(iter(distinct), "standard")
...
def runtime_policy(
    ...
    *,
    ...
    return RuntimePolicy(selected_model, selected_effort, selected_tier)
...
def update_runtime_config(config_text: str, policy: RuntimePolicy) -> tuple[str, dict]:
    updated = replace_top_level_scalar(config_text, "model", policy.model)
    updated = replace_top_level_scalar(updated, "model_fast", policy.model)
    ...
    validate_runtime_config(parsed, policy, "Updated")
    return updated, parsed


def validate_runtime_config(config: dict, policy: RuntimePolicy, source_name: str) -> None:
    expected_tier = "fast" if policy.fast_mode else None
    if config.get("model") != policy.model:
        raise FastMigrationError(f"{source_name} model was not synchronized")
    if config.get("model_fast") != policy.model:
        raise FastMigrationError(f"{source_name} fast model differs from model")
...
def build_provider_plan(row: sqlite3.Row, policy: RuntimePolicy) -> dict:
    ...
    updated_config_text, updated_config = update_runtime_config(config_text, policy)
...
def verify_updated_rows(
    ...
    for plan in plans:
        ...
        validate_runtime_config(config, policy, f"Provider {plan['id']}")
...
def policy_matches(state: dict, policy: RuntimePolicy) -> bool:
    return (
        state["model"] == policy.model
        and state["modelFast"] == policy.model
...
def public_report(
    ...
    matching_count = sum(policy_matches(plan["after"], policy) for plan in plans)
    return {
        "ok": True,
        "mode": mode,
        ...
        and policy_matches(config_after, policy),
        "globalConfig": {
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Synchronize one Codex runtime policy to global config and every provider."
    )
